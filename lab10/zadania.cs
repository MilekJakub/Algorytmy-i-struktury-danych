...
namespace lab_10_task
{
    public class App
    {
        public static void AppMain()
        {
            ...
            {
                Value = new File("game", 0), Children = new List<Node<File>>()
                {
            ...
            {
                Value = new File("app", 0), Children = new List<Node<File>>()
                {
            ...
            try
            {
                ...
                {
                    Console.WriteLine("size =" + files.GetSize());
    ...
    public class Tree<T>
    {
        ...
        private void Preorder(Node<T> node, Action<Node<T>> action)
        {
            ...
            if (node.Children != null)
            {
                ...
                {
                    Preorder(node.Children[i], action);
                }
            }
        }
        ...
        private void PostOrder(Node<T> node, Action<Node<T>> action)
        {
            ...
            if (node.Children != null)
            {
                for (int i = 0; i < node.Children.Count; i++)
                {
                    PostOrder(node.Children[i], action);
                }
            }
        }
    ...
    public class FileSystem : Tree<File>
    {
        ...
        {
            int size = 0;
            PreorderTraversal(n => size += Root.Value.Size);
            PreorderTraversal(n => Console.WriteLine(n.Value.Size));
            //PostorderTraversal(n => count += 1)
            return size;
        }
